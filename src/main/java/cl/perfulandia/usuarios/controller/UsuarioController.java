...
public class UsuarioController {
    ...
    @ApiResponse(responseCode = "200", description = "Lista obtenida exitosamente", content = @Content(mediaType = "application/json", schema = @Schema(implementation = Usuario.class)))
    @GetMapping
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Usuario registrado exitosamente", content = @Content(mediaType = "application/json", schema = @Schema(implementation = Usuario.class))),
            @ApiResponse(responseCode = "400", description = "Datos inválidos")
    })
    @PostMapping("/auth/registro")
    public ResponseEntity<Usuario> registrar(
            @RequestBody(description = "Datos del usuario y nombre del rol", required = true) UsuarioRegistroRequest request) {
    ...
    public ResponseEntity<Usuario> login(
            @RequestBody(description = "Credenciales de login", required = true) LoginRequest request) {
    ...
    public ResponseEntity<Usuario> actualizarUsuario(
            ...
            @RequestBody(description = "Nuevos datos del usuario", required = true) Usuario datosNuevos) {
